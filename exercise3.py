...
class FrequencyModel(tf.Module):
    def __init__(self, N, M, **kwargs):
        super().__init__(**kwargs)
        np.random.seed(42)
        self.N = N
        self.a = tf.Variable(np.random.rand(N,M), name ="a", shape=(N,M), dtype=tf.float32)
        self.b = tf.Variable(np.random.rand(N,M), name ="b", shape=(N,M), dtype=tf.float32)
        self.c = tf.Variable(np.transpose(np.array([[-3.0],[1.0]])), name ="c", shape=(1,M), dtype=tf.float32)
        self.n = tf.stack([tf.range(1,N+1, dtype=tf.float32) for i in range(M)], 1)
    
    def __call__(self, x):
        return self.c + tf.reduce_sum(self.a*tf.sin(math.pi*x*self.b), axis = 0)
...
if __name__ == "__main__":
    with open('data/exercise3.json') as json_file:
